...
public static function findOrCreateContact($properties)
{
    ...
    if (!$contactId) {
        ...
        // Добавляем телефон и email в поля для создания
        if ($phone) {
        ...
        $contactId = self::createContact($contactFields);

        self::writeLog('Create result: ' . ($contactId ? "success, ID: {$contactId}" : 'failed'));

        if (!$contactId) {
            self::writeLog('Error creating contact - returning null');
            return null;
        }
...
{
    try {
        self::writeLog('Attempting to create contact with CCrmContact class');
        self::writeLog('Original fields: ' . print_r($fields, true));

        // Подготавливаем поля в правильном формате
        $contactFields = [
            'NAME' => $fields['NAME'] ?? '',
            'LAST_NAME' => $fields['LAST_NAME'] ?? '',
            'SECOND_NAME' => $fields['SECOND_NAME'] ?? '',
            'OPENED' => 'Y', // Доступен для всех
            'ASSIGNED_BY_ID' => $fields['ASSIGNED_BY_ID'] ?? 1,
        ];
        ...
        if (isset($fields['COMPANY_ID']) && !empty($fields['COMPANY_ID'])) {
            $contactFields['COMPANY_ID'] = $fields['COMPANY_ID'];
        }

        self::writeLog('Prepared contact fields: ' . print_r($contactFields, true));

        // Создаем экземпляр класса контакта
        $bCheckRight = false; // не проверять права для автоматических операций
        $contactEntity = new \CCrmContact($bCheckRight);

        // Добавляем контакт
        $contactId = $contactEntity->Add(
            $contactFields,
            true, // обновлять поисковый индекс
            [
                'CURRENT_USER' => 1, // от имени пользователя с ID 1
            ]
        );

        if ($contactId) {
            self::writeLog("Contact created successfully with ID: {$contactId}");
            
            return $contactId;
        } else {
            $errorMessage = $contactEntity->LAST_ERROR;
            self::writeLog("Failed to create contact. Error: {$errorMessage}");
            return false;
        }
    ...
    private static function findContactByPhone($phone, $companyId = null)
    {
        ...
        if ($companyId) {
            //  $companyId = CompanyManager::findCompanyByTargetID($companyId);
